...
| Route segments gameplay | Primera versión jugable (18C). Las rutas pueden recorrerse por segmentos internos. La marcha avanza automáticamente de segmento a segmento sin botón intermedio. Al completar todos los segmentos, la caravana se mueve al sector destino, se aplica discovery/mastery allí, y el Diario registra "Ruta completada · NOMBRE". El combate dentro de un segmento no rompe el progreso de ruta. |
...
**Avisos globales de expedición (21A.1):** La expedición continúa aunque el jugador esté revisando mapa, inventario, códice o diario. Cuando se completa un tramo o una ruta, el juego lo comunica mediante un dock flotante no intrusivo (`ExpeditionNoticeDock`), coherente con la estética medieval. El aviso no bloquea la interfaz y permite saltar a mapa o caravana si el jugador quiere ver el avance. El aviso de tramo se suprime en la pestaña Caravana (LiveMarchScene ya muestra la transición). El aviso de ruta completada tiene prioridad máxima y permanece hasta que el jugador lo cierre. Durante combate, los avisos de tramo se suprimen; los de ruta se muestran por encima del dock de combate. Los tramos de `route_aethel_to_mist` incluyen ahora datos `visual`, `gameplay` y `completion` con microtextos propios de finalización.

**Mapa Vivo (21A):** El mapa de capa (`viewLevel: 'layer'`) muestra ahora una red densa de caminos orgánicos en vez de líneas simples. Para `stratum_01`, cinco redes visuales (`LAYER_ROUTE_NETWORKS`) con 7–10 puntos cada una cubren la totalidad de los asentamientos: `net_aethel_to_mist`, `net_mist_to_ruins`, `net_aethel_to_salt`, `net_aethel_to_dustgate`, `net_dustgate_to_blindlake`. Cada punto tiene un `kind` (gate, junction, segment, landmark, rest, danger, hidden) que determina el tamaño y color del nodo. Un token de caravana (`MarchPositionMarker`) aparece sobre el camino cuando hay expedición activa, interpolando su posición en función de los tramos completados y el progreso actual (`expedition.progress`). El mismo token aparece en la vista de ruta (`viewLevel: 'route'`), interpolado entre el nodo actual y el siguiente. El pan está ahora contenido en todas las vistas mediante `MAP_PAN_POLICY` y `clampPanForView`. Un botón "Seguir caravana →" aparece en el mapa cuando hay expedición activa.

**Ubicación persistente (21A.2):** La caravana tiene ahora un estado propio de posición física, `currentLocation`, independiente de `expedition.sectorId`. El campo `sectorId` de la expedición reflejaba implícitamente el destino al completar una ruta, pero no era una fuente de verdad explícita para "dónde está la caravana ahora". El nuevo sistema introduce `locationSystem.js` con `createInitialLocation()`, `buildLocationFromRouteDestination()`, `sanitizeLocation()` y otros helpers. El estado se persiste en el save y se carga con retrocompatibilidad (si el campo falta, se reconstruye desde `expedition.sectorId`). CaravanScreen muestra "La caravana descansa en [nombre]" usando `currentLocation.label`. El mapa de capa muestra un marcador estático en reposo (`MarchPositionMarker paused`) sobre el asentamiento que corresponde a `currentLocation.settlementId` cuando no hay expedición activa. `handleAlzar` y `handlePrepareNext` usan `currentLocation.sectorId` como sector de partida real.
